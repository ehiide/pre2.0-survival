...
public class ChatUtil 
{
	...
	public static String addDruggedEmotes(Player player, String message)
	{
		...
		if (DPlayerManager.getInstance().getSerotonine(player.getName()) >= 60)
		{
			...
			if (player.getName().equalsIgnoreCase("ProseczeqPL") && Bukkit.getPlayer("niemilusia") != null && Objects.requireNonNull(Bukkit.getPlayer("niemilusia")).isOnline())
			{
				if (MathUtil.chanceOf(5))
			...
			if (player.getName().equalsIgnoreCase("niemilusia") && Bukkit.getPlayer("ProseczeqPL") != null && Objects.requireNonNull(Bukkit.getPlayer("ProseczeqPL")).isOnline())
			{
				if (MathUtil.chanceOf(5))
					message = "prosek kc";
				if (MathUtil.chanceOf(5))
	...
	public static ChatColor returnChatColorPlayerColor(Player player)
	{
		if (DPlayerManager.getInstance().getChatColor(player).equalsIgnoreCase("red"))
	...
	public static void sendGangChatMessage(Player player, String message)
	{
		...
		{
			ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Aby pisac na czacie gangu musisz najpierw do niego dolaczyc!");
		...
		{
			ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Twoj gang nie posiada zakupionego czatu, aby go odblokowac odwiedz zakladke ulepszen gangu lub wpisz komende #ffc936/gang ulepsz#fc7474!");
			VisualUtil.showDelayedTitle(player, "#fc7474✖", "", 0, 20, 20);
			VisualUtil.showPlayerParticle(player, Particle.FLAME);
			SoundUtil.playPlayerSound(player, Sound.BLOCK_ANVIL_DESTROY, 4, 4);
			return;
		...
		if (message.length() < 2)
		{
			ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#8c8c8cWprowadz swoja wiadomosc, ktora chcesz wyslac na czat gangu!");
			return;
		}
