...
public class Mute implements CommandExecutor
{
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
    {
        if (sender instanceof Player)
        {
            ...
            {
                ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Wyciszac graczy moze tylko administracja!");
            ...
            if (args.length == 0)
            {
                ...
                for (Player dplayer : Bukkit.getOnlinePlayers())
                    if (dplayer.getName().equalsIgnoreCase(potencial_player))
                    {
                        ...
                        {
                            ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Ten gracz jest juz wyciszony, wiec nie mozesz go wyciszyc!");
                            VisualUtil.showDelayedTitle(player, "#fc7474✖", "", 0, 20, 20);
                            VisualUtil.showPlayerParticle(player, Particle.FLAME);
                            SoundUtil.playPlayerSound(player, Sound.BLOCK_ANVIL_DESTROY, 4, 4);
                            return true;
