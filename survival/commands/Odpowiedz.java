...
public class Odpowiedz implements CommandExecutor
{
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
    {
        if (sender instanceof Player)
        {
            ...
            if (DPlayerCloudManager.getReply(player) == null)
            {
                ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Nie masz do kogo odpowiedziec!");
            ...
            {
                String potencial_player = DPlayerCloudManager.getReply(player).getName();
                ...
                {
                    ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Gracz nie jest online na serwerze!");
                    VisualUtil.showDelayedTitle(player, "#fc7474✖", "", 0, 20, 20);
                    VisualUtil.showPlayerParticle(player, Particle.FLAME);
                    SoundUtil.playPlayerSound(player, Sound.BLOCK_ANVIL_DESTROY, 4, 4);
                    return true;
