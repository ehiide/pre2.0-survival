...
public class Vanish implements CommandExecutor
{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) 
	{
		if (sender instanceof Player)
		{
			...
			{
				ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Najpierw daj dupy adminowi, a potem rob co tylko zechcesz!");
			...
			if (args.length == 0)
			{
				if (DPlayerManager.getInstance().getVanished(player))
				{
					...
					ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#80ff1fFunkcja ukrycia zostala wlaczona!");
				}
				else
				{
					...
					ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Funkcja ukrycia zostala wylaczona!");
				}
			}
			else
			{
				ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Umiesz pisac?! Wystarczy napisac #ffc936/vanish!");
				VisualUtil.showDelayedTitle(player, "#fc7474✖", "", 0, 20, 20);
				VisualUtil.showPlayerParticle(player, Particle.FLAME);
				SoundUtil.playPlayerSound(player, Sound.BLOCK_ANVIL_DESTROY, 4, 4);
			}
