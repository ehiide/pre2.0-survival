...
public class Zakuj implements CommandExecutor
{
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
    {
        if (sender instanceof Player)
        {
            ...
            {
                ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Uwiezic graczy moze tylko administracja!");
            ...
            if (args.length == 0)
            {
                ...
                for (Player dplayer : Bukkit.getOnlinePlayers())
                    ...
                    {
                        if (TimeUtil.getDifferenceInSeconds(DPlayerManager.getInstance().getDungeoned(player.getName())) < DPlayerManager.getInstance().getDungeonLength(player.getName()))
                        {
                            ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Ten gracz jest juz zakuty!");
                            VisualUtil.showDelayedTitle(player, "#fc7474✖", "", 0, 20, 20);
                            VisualUtil.showPlayerParticle(player, Particle.FLAME);
                            SoundUtil.playPlayerSound(player, Sound.BLOCK_ANVIL_DESTROY, 4, 4);
                            return true;
                        ...
                        {
                            ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Podany czas nie jest obslugiwany, naucz sie cyfr!");
                            VisualUtil.showDelayedTitle(player, "#fc7474✖", "", 0, 20, 20);
                            VisualUtil.showPlayerParticle(player, Particle.FLAME);
                            SoundUtil.playPlayerSound(player, Sound.BLOCK_ANVIL_DESTROY, 4, 4);
                            return true;
                        ...
                        {
                            ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Podany format czasowy nie jest obslugiwany!");
                            VisualUtil.showDelayedTitle(player, "#fc7474✖", "", 0, 20, 20);
                            VisualUtil.showPlayerParticle(player, Particle.FLAME);
                            SoundUtil.playPlayerSound(player, Sound.BLOCK_ANVIL_DESTROY, 4, 4);
                            return true;
                        ...
                        if (args.length == 2)
                        {
                            ...
                            DPlayerManager.getInstance().setDungeoned(dplayer, date);
                            DPlayerManager.getInstance().setDungeonLength(dplayer, catchTime(init));
                            ...
                            dplayer.teleport(WorldUtil.LOBBY_DUNGEON);
