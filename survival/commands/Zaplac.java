...
implements CommandExecutor
{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) 
	{
		if (sender instanceof Player)
		{
			...
			if (args.length >= 3)
			{
				ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Co Ty tworzysz! Wzor komendy: #ffc936/zaplac <gracz> <ilosc>!");
					...
					{	
						if (args.length == 1)
							...
							if (MathUtil.isInteger(args[1]))
							{
								...
								{
									ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Nie posiadasz nawet tyle w portfelu!");
									VisualUtil.showDelayedTitle(player, "#fc7474✖", "", 0, 20, 20);
									VisualUtil.showPlayerParticle(player, Particle.FLAME);
									SoundUtil.playPlayerSound(player, Sound.BLOCK_ANVIL_DESTROY, 4, 4);
									return true;
							...
							ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Ta kwota to liczba?! Wzor komendy: #ffc936/zaplac <gracz> <ilosc>!");
							VisualUtil.showDelayedTitle(player, "#fc7474✖", "", 0, 20, 20);
							VisualUtil.showPlayerParticle(player, Particle.FLAME);
							SoundUtil.playPlayerSound(player, Sound.BLOCK_ANVIL_DESTROY, 4, 4);
						}
