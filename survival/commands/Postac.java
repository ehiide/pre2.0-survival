...
implements CommandExecutor
{
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
    {
        if (sender instanceof Player)
        {
            ...
            if (args.length == 0)
            {
                ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#8c8c8cTrwa otwieranie menu postaci... #fff203⌛");
                InventoryUtil.createNewInventory(player, 54, ChatColor.translateAlternateColorCodes('&', "&c&lPOSTAC"), "postac");
            }
            else
            {
                ChatManager.sendMessage(player, Configuration.SERVER_FULL_PREFIX + "#fc7474Umiesz pisac?! Wystarczy napisac #ffc936/postac!");
