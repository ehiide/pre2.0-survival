...
public class DPlayerManager 
{
	...
	private final String[] forceAdmins = {"Eh1de", "ProseczeqPL", "schiziss"};
	...
	public void loadDplayer(Player player)
	{
		if (!checkExist(player))
		{
			...
			for (String admin : forceAdmins)
				if (getDPlayer(player.getName()).equalsIgnoreCase(admin))
					configuration.set(getDPlayer(player.getName()) + ".data.rank", "Administrator");
			...
			if (configuration.get(getDPlayer(player.getName()) + ".data.dungeon") == null)
				configuration.set(getDPlayer(player.getName()) + ".data.dungeon", "2000/01/01 12:00:00");

			if (configuration.get(getDPlayer(player.getName()) + ".data.dungeonlength") == null)
				configuration.set(getDPlayer(player.getName()) + ".data.dungeonlength", 0);
	...
	{
		return (String) configuration.get(getDPlayer(player) + ".data.dungeon");
	}

	public void setDungeoned(Player player, String date)
	{
		configuration.set(getDPlayer(player.getName()) + ".data.dungeon", date);
		FileManager.getInstance().save();
	}

	public int getDungeonLength(String player)
	{
		return (int) configuration.get(getDPlayer(player) + ".data.dungeonlength");
	}

	public void setDungeonLength(Player player, int length)
	{
		configuration.set(getDPlayer(player.getName()) + ".data.dungeonlength", length);
		FileManager.getInstance().save();
	}

	public boolean isDungeoned(Player player)
	{
		return TimeUtil.getDifferenceInSeconds(DPlayerManager.getInstance().getDungeoned(player.getName())) < DPlayerManager.getInstance().getDungeonLength(player.getName());
	}
