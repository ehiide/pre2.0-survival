...
public class NeuroManager
{
    ...
    public static void apply(Player player)
    {
        ...
        PacketUtil.cancelTask(player, PacketUtil.uncoordinatedEffect);
        ...
        else if (noradrenaline <= 50 && noradrenaline > 30)
        {
            speed = speed + 0.13F;
            if (PacketUtil.getShakeEffect(player) == null)
                PacketUtil.shakeCamera(player, 2, 0.005);
            nightvision = true;
            glowing = true;
            saturation = true;
            haste = haste + 1;
        ...
        if (gaba < Integer.MAX_VALUE && gaba > 50)
        {
            ...
            PacketUtil.uncoordinatedEffect(player, 10);
    ...
    {
        return DPlayerManager.getInstance().getSerotonine(player.getName()) != 0 || DPlayerManager.getInstance().getDopamine(player.getName()) != 0 ||
                DPlayerManager.getInstance().getNoradrenaline(player.getName()) != 0 || DPlayerManager.getInstance().getGABA(player.getName()) != 0;
    }
}